...
{
    ros::init(argc,argv, "FalconJoystick1");
    ...
    node.param<int>("falcon_number1", falcon_int, 1);
    node.param<bool>("falcon_debug1", debug, false);
    node.param<bool>("falcon_clutch1", clutchPressed, true);
    node.param<bool>("falcon_coag1", coagPressed, true);
    cout<< falcon_int <<endl;
    ...
    if(init_falcon(falcon_int))
    {
        ...
        ros::Subscriber sub = node.subscribe("/falconForce1", 10, &forceCallback);


        //Start ROS Publisher
        ros::Publisher pub = node.advertise<sensor_msgs::Joy>("/falcon/joystick1",10);

...
xixtilde_store = np.empty((2,0))
xiyptilde_store = np.empty((2,0))
xiythtilde_store = np.empty((2,0))
    ...
    try:
        ...
        while not rospy.is_shutdown():
            if(time.time() - prev_time >= dt):
                ...
                # print("Current state: {} and Desired state: {}".format(pos_box,des_pos))
                ...
                xixh= xixh+np.multiply(dt,np.matmul(Ax,xixh)+np.multiply(Bx,uxHh)-np.matmul(Gammax,xixtilde))
                xiyph= xiyph+np.multiply(dt,np.matmul(Ay,xiyph)+np.multiply(By,uysHh)-np.matmul(Gammayp,xiyptilde))
                xiythh=xiythh+np.multiply(dt,np.matmul(Ay,xiythh)+np.matmul(By,uydHh)-np.matmul(Gammayth,xiythtilde))
                ...
                left_falcon(-flH[0],-flH[1],-flH[2])#(-0,0,0)#(-flH[0]+flR[0]+frb[0],-flH[1]+flR[1]+frb[1],-flH[2]+flR[2]+frb[2])
                right_falcon(-frH[0],-frH[1],-frH[2])#(0,0,0)#(-frH[0]+frR[0]+flb[0],-frH[1]+frR[1]+flb[1],-frH[2]+frR[2]+flb[2])
                #print(np.matmul(np.matmul(By,uyrH-uylH)))
                xith=xith+np.multiply(dt,np.matmul(Ay,xith)+np.matmul(By,np.matrix(uyrH-uylH)))
                X = np.matrix([[pos_box[0]],[vel_box[0]]])
                Y = np.matrix([[pos_box[1]],[vel_box[1]]])
                Z = np.matrix([[pos_box[2]],[vel_box[2]]])
                xix=X-Xd
                ...
                xixtilde_store= np.append(xixtilde_store, xixtilde, axis=1)
                xiyptilde_store= np.append(xiyptilde_store, xiyptilde, axis=1)
                xiythtilde_store= np.append(xiythtilde_store, xiythtilde, axis=1)
                ...
                if(curr_time>tf):
                    ...
                    scipy.io.savemat('xixtilde_.mat', {'xixtilde_': xixtilde_store})
                    scipy.io.savemat('xiyptilde_.mat', {'xiyptilde_': xiyptilde_store})
                    scipy.io.savemat('xiythtilde_.mat', {'xiythtilde_': xiythtilde_store})
                    left_falcon(0,0,0)
                    right_falcon(0,0,0)
                    break

    ...
    try:
        ...
        while not rospy.is_shutdown():
            if(time.time() - curr_time >= dt):
                ...
                xixh= xixh+np.multiply(dt,np.matmul(A,xixh)+np.matmul(B,uxHh)-np.matmul(Gammax,xixtilde))
                xiyph= xiyph+np.multiply(dt,np.matmul(A,xiyph)+np.matmul(B,uysHh)-np.matmul(Gammayp,xiyptilde))
                xiythh=xiythh+np.multiply(dt,np.matmul(A,xiythh)+np.matmul(B,uydHh)-np.matmul(Gammayth,xiythtilde))
                xizph= xizph+np.multiply(dt,np.matmul(Az,xizph)+np.matmul(Bz,uzsHh)-np.matmul(Gammazp,xizptilde))+cz
                xizthh=xizthh+np.multiply(dt,np.matmul(Az,xizthh)+np.matmul(Bz,uzdHh)-np.matmul(Gammazth,xizthtilde))+cz
                ...
                xithy=xithy+np.multiply(dt,np.matmul(A,xithy)+np.matmul(B,np.matrix(uyrH-uylH)))
                xithz=xithz+np.multiply(dt,np.matmul(Az,xithz)+np.matmul(Bz,np.matrix(uzrH-uzlH)))+ cz
                X = np.matrix([[pos_box[0]],[vel_box[0]]])
                Y = np.matrix([[pos_box[1]],[vel_box[1]]])
                Z = np.matrix([[pos_box[2]],[vel_box[2]]])
                xix=X-Xd

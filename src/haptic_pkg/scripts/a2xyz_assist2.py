...
# State Variables
pos_falcon_left = np.array([0, 0, 0])
...
def box_pos_cb(data):
    ...
    try:
        ...
        rospy.logwarn("floating_box::base_link not found in /gazebo/link_states")
...
try:
    while not rospy.is_shutdown():
        ...
        # 3D square-like path (same shape, 10s per leg)
        if t < 10:

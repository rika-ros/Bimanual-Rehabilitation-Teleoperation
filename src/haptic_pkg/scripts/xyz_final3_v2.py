...
if __name__ == "__main__":
    try:
		...
		pos_box_list.append(list(pos_box))
		vel_box_list.append(list(vel_box))
		zd_list.append(Zd[0,0])
                xithz = THz-THdz
                uxlH=np.matrix([[0]])
                uylH=np.matrix([[0]])
...
savemat('xyz_final3_data.mat',data_dict)
rospy.loginfo("saved data in xyz_final3_data.mat")
...
def box(force_x,force_y,force_z):  
    ...
    msg_force_to_box.force.y = force_y
    msg_force_to_box.force.z = force_z
    force_Box.publish(msg_force_to_box)
...
rospy.Subscriber("/falcon/joystick1", Joy, callback_left_falcon, queue_size=1)
rospy.Subscriber("/left/force_feedback", WrenchStamped, callback_left_box, queue_size=1)
rospy.Subscriber("/right/force_feedback", WrenchStamped, callback_right_box, queue_size=1)
...
if __name__ == "__main__":
    ...
    savemat('xyz_unassist_data.mat', data_dict)
    rospy.loginfo("Saved data to xyz_unassist_data.mat")'''

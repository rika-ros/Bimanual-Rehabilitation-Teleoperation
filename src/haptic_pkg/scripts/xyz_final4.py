...
if __name__ == "__main__":
    try:
        ...
        last_phase = -1  # Used to track whether to print "Lift Up" or "Bring Down"
        amplitude = 0.2  # Amplitude of vertical motion (meters)
        omega = np.pi / 10  # Angular frequency: one full cycle in 20s
	...
	while not rospy.is_shutdown():
        ...
        Zd_val = amplitude * np.sin(omega * curr_time)  # sin wave over 20s
        Zd = np.matrix([[Zd_val], [0]])

        # Phase print only once per half-cycle
        if np.sin(omega * curr_time) > 0 and last_phase != 0:
            print("Lift Up Phase")
            test = 0
            last_phase = 0
        elif np.sin(omega * curr_time) <= 0 and last_phase != 1:
            print("Bring Down Phase")
            test = 1
            last_phase = 1

        # --- Rest of your control code continues here ---
        # No need to change your uzlH, uzrH, uzlR, uzrR calculation logic
        # Also use the existing Falcon force logic block

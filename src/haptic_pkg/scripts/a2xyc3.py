...
QyR=np.diag([0.1,0])       #Robot parameters
QxR=np.diag([0.05,0])
    ...
    try:
        ...
        while not rospy.is_shutdown():
            if(time.time()-program_starts - prev_time >= dt):
                ...
                flR=np.matrix([uxlR,[uylR],[0]])#net force vector for robot
                ...
                left_box(flb[0]+flR[0],flb[1]+flR[1],flb[2]+flR[2])#
                right_box(frb[0]+frR[0],frb[1]+frR[1],frb[2]+frR[2])
                # if(prev_time<3):
                #     left_box(flb[0],flb[1],flb[2])#
                #     right_box(frb[0],frb[1],frb[2]) #
                # else:

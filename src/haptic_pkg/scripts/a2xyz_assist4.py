...
#intialize
# Wait until box position is updated by Gazebo (from /gazebo/link_states)
rospy.loginfo("Waiting for valid box position from Gazebo...")
while np.all(pos_box == 0) and not rospy.is_shutdown():
    rospy.sleep(0.1)
rospy.loginfo("Received box position. Starting control loop.")
...
if __name__ == "__main__":
    try:
		...
		LxrR = 0.5 * LxR
                ...
                LylR=LyR-LyrR
                uyrR = -0.5 * np.matmul(LyrR, xiyp)
		uylR = -0.5 * np.matmul(LylR, xiyp)

		uyrR = np.clip(uyrR, -15, 15)
		uylR = np.clip(uylR, -15, 15)
                ...
                kzlR=kzR-kzrR
                uzrR = -0.5 * (np.matmul(LzrR, xizp) + np.matmul(Bz.T, kzrR))
		uzlR = -0.5 * (np.matmul(LzlR, xizp) + np.matmul(Bz.T, kzlR))
                ...
                #     print(xiythh)
                #     print(xizphih)
		norm = np.linalg.norm(xix)**2 + 1e-6  # add epsilon to avoid division by zero
		PxHh = PxHh + dt * np.matmul(np.matmul(alphax, xixtilde), xix.T) / norm
		PxHh = np.clip(PxHh, -1e6, 1e6)

                norm_yp = np.linalg.norm(xiyp)**2 + 1e-6
		norm_yth = np.linalg.norm(xiyp)**2 + 1e-6
		PysHh = PysHh + dt * np.matmul(np.matmul(alphayp, xiyptilde), xiyp.T) / norm_yp
		PydHh = PydHh + dt * np.matmul(np.matmul(alphayth, xiythtilde), xiyp.T) / norm_yth
...
# Clip to prevent explosion
		PysHh = np.clip(PysHh, -1e6, 1e6)
		PydHh = np.clip(PydHh, -1e6, 1e6)
		...
		print("===== DEBUG FRAME @ t = {:.2f}s =====".format(prev_time))

# Falcon positions and deltas
		print("Falcon Left Pos:", pos_falcon_left)
		print("Falcon Right Pos:", pos_falcon_right)
		print("Delta Left (box - falcon):", pos_box[0] - pos_falcon_left[0]*50)
		print("Delta Right (box - falcon):", pos_box[0] - pos_falcon_right[0]*50)

# Box state
		print("Box Pos: [{:.3f}, {:.3f}, {:.3f}]".format(pos_box[0], pos_box[1], pos_box[2]))
		print("Box Vel: [{:.3f}, {:.3f}, {:.3f}]".format(vel_box[0], vel_box[1], vel_box[2]))

# Raw human forces from error
		print("flH (left user force):", flH.T)
		print("frH (right user force):", frH.T)

# AAN assist forces
		print("uxlR (left assist X):", uxlR)
		print("uxrR (right assist X):", uxrR)
		print("uylR (left assist Y):", uylR)
		print("uyrR (right assist Y):", uyrR)
		print("uzlR (left assist Z):", uzlR)
		print("uzrR (right assist Z):", uzrR)

# Total force sent to Falcon devices
		print("Force sent to Left Falcon: [{:.2f}, {:.2f}, {:.2f}]".format(
    float(-flH[0, 0] + uxlR[0, 0]),
    float(-flH[2, 0] + uzlR[0, 0]),
    float(-flH[1, 0] + uylR[0, 0])
))
		print("Force sent to Right Falcon: [{:.2f}, {:.2f}, {:.2f}]".format(
    float(-frH[0, 0] + uxrR[0, 0]),
    float(-frH[2, 0] + uzrR[0, 0]),
    float(-frH[1, 0] + uyrR[0, 0])
))
                print("Reference Pos (xr, yr, zr): {:.3f}, {:.3f}, {:.3f}".format(
    float(Xd[0, 0]), float(Yd[0, 0]), float(Zd[0, 0])))
        ...
        pass
    # rospy.spin()
'''| Time (s) | x\_d (m) | y\_d (m) | z\_d (m) | Motion Description                       |
| -------- | -------- | -------- | -------- | ---------------------------------------- |
| 0-10     | 0 - 0.8  | 0        | 0        | Move right along **x**                   |
| 10-20    | 0.8      | 0        | 0 - 0.8  | Lift up along **z**                      |
| 20-30    | 0.8      | 0 - 0.8  | 0.8      | Move forward along **y**                 |
| 30-40    | 0.8      | 0.8      | 0.8 - 0  | Lower down along **z**                   |
| 40-50    | 0.8 - 0  | 0.8      | 0        | Move left along **x**                    |
| 50-60    | 0        | 0.8      | 0 - 0.8  | Lift up along **z**                      |
| 60-70    | 0        | 0.8 - 0  | 0.8      | Move backward along **y**                |
| 70-80    | 0        | 0        | 0.8 - 0  | Lower down along **z** (return to start) |
'''
